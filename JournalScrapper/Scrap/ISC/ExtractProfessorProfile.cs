    ...
    public static async Task ScrapProfessorProfile()
    {
        ...
        foreach (var url in urls)
        {
            try
            {
                ...
                professor.Articles = professor.Articles.TakeWhile(article => !article.Title.Contains("Book") && article.Title.Split(" ").Length < 5).ToList();
                ...
                professor.ProfessionalActivities = GetElements(professionalActivitiesElement,
                    ...
                    .Select(x => new ProfessionalActivity { Title = x.Text, ProfessorProfileId = professor.Id }).ToList();
