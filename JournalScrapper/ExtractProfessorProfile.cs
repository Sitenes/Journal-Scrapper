    ...
    public static async Task ScrapProfessorProfile()
    {
        ...
        foreach (var url in urls)
        {
            try
            {
                ...
                professor.FullName = titleName?.Split(",").FirstOrDefault("") ?? "";
                ...
                if (string.IsNullOrEmpty(professor.FullName))
                    continue;
                var exist = await _context.ProfessorProfiles.AnyAsync(x => x.FullName.Equals(professor.FullName) && x.Department.Equals(professor.Department));
                if (exist)
                    continue;
                ...
                var WebLinks = ScrapeWebLinks(professor.Id);
                await _context.AddRangeAsync(WebLinks ?? new List<WebLink>());

                var Educations = ScrapeEducations(professor.Id);
                await _context.AddRangeAsync(Educations ?? new List<Education>());

                var ResearchAreas = ScrapeResearchAreas(professor.Id);
                await _context.AddRangeAsync(ResearchAreas ?? new List<ResearchArea>());

                //var Articles = 
                //await _context.AddRangeAsync(Articles);

                var TeachingInterests = ScrapeTeachingInterests(professor.Id);
                await _context.AddRangeAsync(TeachingInterests ?? new List<TeachingInterest>());

                var Books = ExtractBooks(professor.Id);
                await _context.AddRangeAsync(Books ?? new List<Book>());
                ...
                var Courses = badgeElements?.Select(x => new Course
                {
                    Title = x.Text,
                }).ToList();
                await _context.AddRangeAsync(Courses ?? new List<Course>());
                ...
                var ProfessionalActivities = GetElements(professionalActivitiesElement,
                ...
                await _context.AddRangeAsync(ProfessionalActivities ?? new List<ProfessionalActivity>());

                var Memberships = GetElements(coursesElement,
                ...
                await _context.AddRangeAsync(Memberships ?? new List<Membership>());
                ...
                await _context.AddRangeAsync(Links ?? new List<ProfessorLink>());
